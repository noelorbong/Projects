var humiChart =null;
  ...
  function handler(){
      ...
      humirequestData(startTime, endTime);
  ...
  function requestData(){
    ...
    var time = [];
    var datetime =[];
    $.ajax({url: "/realtimehumi/", success: function(responseData){
  ...
  function humirequestData(startTime, endTime){
     ...
     // console.log("Date:" + date)
      $.ajax({url: "/selectedhumidate/"+startTime.toString()+"/"+endTime.toString(), success: function(responseData){
          $.each(responseData.humidities, function(i, item) {
              id[i] = item.id;
              value[i] = item.value;
              time[i] = " "+(item.created_at).slice(11)+" ";
  ...
  function displayChart(value, time){
    ...
    var pieChartContent = document.getElementById('chartContent');
        ...
        $('#chartContent').append('<canvas id="humiChart"  width="100%" height="40vh" ><canvas>');

    var ctx = document.getElementById("humiChart").getContext('2d');//$("#tempChart").get(0).getContext("2d"); 
    //
