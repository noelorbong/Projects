...
void fanControl(){
  if(temperature >= 29){
    if(isFanOn == true){
      ...
      char fanCont[20];
      String fanCon = "ctrfn 1";
      fanCon.toCharArray(fanCont, 20);
      client.publish("icuMonitoring/data",fanCont);
    }
  }else{
    if(isFanOn == false){
      digitalWrite(FAN, LOW);
...
void alarmControl(){
  if(co2 >= 1000){
    if(isAlarmOn == true){
      ...
      char alarmCont[20];
      String alarmCon = "ctral 1";
      alarmCon.toCharArray(alarmCont, 20);
      client.publish("icuMonitoring/data",alarmCont);
    }
  }else{
    if(isAlarmOn == false){
      digitalWrite(ALARM, LOW);
      isAlarmOn = true;
      Serial.println("ctral is off!");
      char alarmCont[20];
      String alarmCon = "ctral 0";
      alarmCon.toCharArray(alarmCont, 20);
      client.publish("icuMonitoring/data",alarmCont);
    }
  }
